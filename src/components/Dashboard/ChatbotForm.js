...
const ChatbotForm = ({ initialData = {}, mode = 'create' }) => {
    ...
    const removeImage = (index) => {
        ...
        setProjectImagesURL((prevImages) => prevImages.filter((_, i) => i !== index));
    };

    useEffect(() => {
        const fetchChatbotData = async () => {
            if(Object.keys(initialData).length > 0)
            console.log('Fetching chatbot data', initialData);
            try {
                if (mode === 'update' && Object.keys(initialData).length > 0){
                    setChatbotId(initialData?._id)
                    setName(initialData?.name)
                    setWebhook(initialData?.webhook)
                    const prevContent = initialData?.button_content;
                    setButtonContent(prevContent)
                    // setProjectImages(initialData?.projectImages);
                    // setProjectLogo(initialData?.projectLogo);
                    setProjectLogoURL(initialData?.projectLogo);
                    setProjectImagesURL(initialData?.projectImages);
                }
            } catch (err) {
    ...
    return (
        <div className="create-chatbot-container d-flex justify-content-center align-items-center vh-100">
            <div className="create-chatbot-card p-4 shadow rounded bg-white">
                <h1 className="text-center text-primary mb-4">{mode === 'create' ? 'Create Chatbot' : 'Update Chatbot'}</h1>
